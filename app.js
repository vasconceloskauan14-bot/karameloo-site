...
function brl(v){ return Number(v||0).toLocaleString('pt-BR', {style:'currency', currency:'BRL'}); }
    ...
    function isClientLogged(){
      // Fonte principal: sessão (localStorage)
      const s = getSession();
...
<h4>8) Retenção</h4>
<p>Guardamos dados pelo tempo necessário para operar a plataforma e cumprir obrigações legais. Em beta/localStorage, a retenção depende do seu navegador.</p>
...
let arr = all.slice();
    ...
    function openProcurarApiConfig(){
      ...
      localStorage.setItem(LS_API_BASE, '');
...
;
(() => {
  // efeitos de fundo/parallax desativados para evitar bugs
  return;
...
function cancelPaymentAndClose(){
  try{
    ...
    if(o){
      ...
      // salva no localStorage se existir lista
      try{
        const arr = lsGet(LS_ORDERS, []);
        const list = Array.isArray(arr) ? arr : [];
...
(function(){
  ...
  // restore tab
  const savedTab = localStorage.getItem('kar_corner_tab') || 'motion';
  cornerSetTab(savedTab, true);
  const savedSort = localStorage.getItem('kar_corner_sort');
...
function cornerToggleMenu(force){
  ...
  try{ localStorage.setItem('kar_corner_menu_open', next ? '1' : '0'); }catch(e){}
}
...
(function(){
  ...
  if(!el) return;
  let v = null;
  try{ v = localStorage.getItem('kar_corner_menu_open'); }catch(e){}
  if(v === '1') el.classList.add('menuOpen');
})();

function cornerSetTab(tab, silent){
  try{ localStorage.setItem('kar_corner_tab', tab); }catch(e){}
...
function cornerApplySort(v){
  try{ localStorage.setItem('kar_corner_sort', v); }catch(e){}
...
(function(){
  ...
  window.createOrderAndFinish = function(editor, overrides){
    ...
    try{ localStorage.setItem('kml_last_order', JSON.stringify(order)); }catch(e){}
